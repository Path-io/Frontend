      ...
      <div className="bg-primary-foreground md:px-[6%] hidden md:flex w-1/2 gap-8 flex-col items-center justify-center">
        ...
        <div className="flex flex-col gap-8">
          ...
          <ul className="flex flex-col justify-center gap-3">
            {pros.map((pro, index) => (
              <li key={index} className="flex gap-2"> <Image width={24} height={24} alt="List Item" src={"/images/listCheck.svg"} /> {pro} </li>
            ))}
          </ul>
          <hr />
          <p>No distractions, no hidden costs — just a clear path to knowledge. Path.io makes my learning so much easier!</p>
          <hr />
          <p className="text-muted-foreground text-sm"> &copy; {new Date().getFullYear()} Path.io </p>
        </div>
      </div>
          ...
          <div className="form w-full px-[10%] md:px-[20%] space-y-8 text-foreground">
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 w-full">
                <CustomField control={form.control} formValue="email" placeholder="team@pathio.com" />
                <CustomField control={form.control} formValue="password" placeholder="••••••••••" />
                <Button type="submit" className="w-full">Login</Button>
                <Button type="submit" variant={"outline"} className="w-full" >Login with Google</Button>
              </form>
            </Form>
            <div className="footer text-sm flex flex-col gap-4">
              <p>Don't have an account? <Link className="underline" href={"/register"}>Sign up</Link></p>
              <Link className="hover:underline" href={"/forgot"}>Forgot your password?</Link>
            </div>
          </div>
      </div>
    </div>
